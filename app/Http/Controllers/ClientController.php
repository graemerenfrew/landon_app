...
use App\Client as Client; //import the model

class ClientController extends Controller
{
    //
    public function __construct( Title $titles, Client $client ) //dependency injection
    {
        $this->titles = $titles->all();  //this is putting the injected $titles into a variable called titles that we can use as we want
        $this->client = $client; //this is doing similar
    }
    ...
    public function newClient( Request $request, Client $client)  //inject the request and client instances
    {
        $data = [];
        ...
        if ( $request->isMethod('post' ))
        {
            ...
            $client->insert($data); //we can just use $data because the element names are the same as the field names
    ...
    public function show($client_id, Request $request)
    {
        ...
        $client_data->last_name );  //session cookie now contains first and last name
        return view('client/form', $data);
    }

    public function modify( Request $request, $client_id, Client $client)  //inject the request and client instances
    {
        $data = [];
        ...
        if ( $request->isMethod('post' ))
        {
            ...
            $client_data = $this->client->find($client_id);//the current record
